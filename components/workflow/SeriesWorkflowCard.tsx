...
const AssetCoverageMatrixPanel: React.FC<AssetCoverageMatrixPanelProps> = ({
  ...
  const coverageStats = useMemo(() => {
    ...
    const uncoveredAssets = assetCoverage.filter(entry => entry.missingCount > 0).length;
    const fullyCoveredAssets = assetCoverage.filter(entry => entry.missingCount === 0).length;
    const averageCoverage = totalAssets === 0
      ? 0
      : Math.round((assetCoverage.reduce((sum, entry) => sum + entry.coverageRate, 0) / totalAssets) * 100);
