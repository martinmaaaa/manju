  ...
  if (nav) {
    return (
      <div className="min-h-screen bg-[#030507] text-white">
        ...
        <div className="mx-auto min-h-screen max-w-[1600px] px-4 pb-8 pt-6 md:px-6 xl:px-8">
          <div className="grid gap-6 xl:grid-cols-[300px_minmax(0,1fr)]">
            <aside className="xl:sticky xl:top-6 xl:self-start">
              <div className="rounded-[28px] border border-white/10 bg-black/30 px-5 py-5 shadow-[0_24px_60px_rgba(0,0,0,0.28)] backdrop-blur">
                <div className="text-[11px] uppercase tracking-[0.36em] text-cyan-300/75">添梯工作流</div>
                <h1 className="mt-3 text-3xl font-semibold tracking-tight text-white">{title}</h1>
                {subtitle ? <p className="mt-3 text-sm leading-7 text-slate-300">{subtitle}</p> : null}
            ...
            <div className="min-w-0">
              {rightSlot ? (
                <div className="rounded-[28px] border border-white/10 bg-black/30 px-5 py-4 shadow-[0_24px_60px_rgba(0,0,0,0.28)] backdrop-blur">
                  <div className="flex flex-wrap items-center gap-3">{rightSlot}</div>
                </div>
              ) : null}
              <main className={rightSlot ? 'pt-6' : ''}>{children}</main>
            </div>
          </div>
        </div>
      </div>
