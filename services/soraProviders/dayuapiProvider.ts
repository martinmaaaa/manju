...
export class DayuapiProvider implements SoraProvider {
  ...
  async submitTask(
    ...
    return logAPICall(
      ...
      async () => {
        // 使用后端代理
        const apiUrl = 'https://api.dayu.com/v1/create';
  ...
  async checkStatus(
    ...
    return logAPICall(
      ...
      async () => {
        // 使用后端代理
        const apiUrl = `https://api.dayu.com/v1/query?id=${encodeURIComponent(taskId)}`;
        const response = await fetch(apiUrl, {
          method: 'GET',
          headers: {
            'X-API-Key': apiKey,
        ...
        if (status === 'completed' && !videoUrl) {
          ...
          try {
            const contentResponse = await fetch(`https://api.dayu.com/v1/content?id=${data.id}`, {
              method: 'GET',
              headers: {
                'X-API-Key': apiKey,
              },
